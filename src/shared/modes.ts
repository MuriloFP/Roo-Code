...
export function isToolAllowedForMode(
	...
	if (experiments) {
		// If the tool itself is an experiment toggle
		if (tool in experiments) {
			if (!experiments[tool]) {
				return false
			}
		}

		// Task card tools require the task_cards experiment to be enabled
		if ((tool === "update_task_card" || tool === "get_task_card") && !experiments["task_cards"]) {
			return false
		}
	}
